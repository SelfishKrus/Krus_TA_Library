#ifndef TEXTURE_TILING_RANDOMIZATION
#define TEXTURE_TILING_RANDOMIZATION
...
TriangleFloat4 SampleTriangleTextures(TEXTURE2D_PARAM(textureName, samplerName), TriangleUV uv, float lod)
{
    TriangleFloat4 result;
    result.r = SAMPLE_TEXTURE2D_LOD(textureName, samplerName, uv.r, lod);
    result.g = SAMPLE_TEXTURE2D_LOD(textureName, samplerName, uv.g, lod);
    result.b = SAMPLE_TEXTURE2D_LOD(textureName, samplerName, uv.b, lod);
    return result;
}
...
float4 SampleTextureRandomizedHexGrid(TEXTURE2D_PARAM(textureName, samplerName), float2 uv, float lod, float hexSize, float edgeContrast)
{
    ...
    TriangleFloat4 triValues = SampleTriangleTextures(TEXTURE2D_ARGS(textureName, samplerName), triUV, lod);
